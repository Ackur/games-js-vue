<template>
  <canvas :ref="(el) => (refs.canvas = el)" id="canvas"></canvas>
</template>
...
function Confetto() {
  ...
  this.width = this.dimensions.x * this.scale.x;
  this.height = this.dimensions.y * this.scale.y;
...
function render() {
  try {
    ...
    confetti.forEach((confetto, index) => {
      if (confetto.position.y - confetto.height >= canvas.height)
    ...
    if (confetti.length || sequins.length) window.requestAnimationFrame(render);
  } catch (error) {
    console.log(error);
    throw error;
  }
